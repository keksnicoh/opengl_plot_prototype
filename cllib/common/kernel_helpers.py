...
def process_arguments_declaration(device, arguments):
    ...
    for i, cl_arg in enumerate(arguments):
        ...
        elif cl_type == np.float64: raise NotImplementedError('double precision behavior not defined yet.')
        elif cl_type == np.complex64: 
            arg_declr[i] = (cl_arg[0], cl_arg[1], 'cfloat_t', cl_arg[3])
            require_complex_numbers = True

        elif cl_type == np.complex128: raise NotImplementedError('auto complex numbers behavior not defined yet.')
